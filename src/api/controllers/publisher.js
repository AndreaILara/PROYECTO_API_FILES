...
const postPublisher = async (req, res, next) => {
  try {
    ...
    if (existingPublisher) {
      return res.status(400).json('Editor already registered');
    }
...
const deletePublisher = async (req, res, next) => {
  try {
    ...
    deletedPublisher
      ? res.status(200).json({ mensaje: 'Editor deleted', deletedPublisher })
      : res.status(404).json('The search data does not exist');
  } catch (error) {
    return res.status(400).json(error);
  }
};

module.exports = { postPublisher, editPublisher, deletePublisher, getPublishers, getPublisherByName };